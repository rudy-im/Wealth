...
def select(tablename, con, selectstr='*', wherestr='', ordercolumn='', asc=False, limit=0):
...
# 테이블 이름 변경
def alterTablename(beforename, con, aftername):
    ...
    try:
        ...
        print("\n\n")
        print("Failed to alterTablename()!!")
        print("--beforename :: " + beforename)
        print("--aftername :: " + aftername)


# 새로운 칼럼을 하나 추가
def addColumn(tablename, con, columnstr):
    ...
    try:
        ...
        print("\n\n")
        print("Failed to addColumn()!!")
        print("--tablename :: " + tablename)
        print("--columnstr :: " + columnstr)


# 테이블 복제
# nodata = True 이면 구조만 복제, False이면 내용도 전부 복제
def copyTable(tablename, con, newname, nodata=True):
    ...
    try:
        ...
        print("\n\n")
        print("[copyTable()] Failed to copyTable()!!!")
...
# 테이블 정렬
# asc = True 이면 오름차순, False이면 내림차순(desc)
def orderTable(tablename, con, ordercolumn, asc=True):

    drop('tmp', con)
    alterTablename(tablename, con, 'tmp')
    copyTable('tmp', con, tablename)

    cursor = con.cursor()

    sql = 'INSERT INTO '
    sql = sql + tablename
    sql = sql + ' SELECT * FROM tmp ORDER BY '
    sql = sql + ordercolumn
    if asc: sql = sql + ' ASC'
    sql = sql + ';'
    ...
    try:
        ...
        print("\n\n")
        print("Failed to orderTable()!!")
        print("--tablename :: " + tablename)
        print("--ordercolumn :: " + ordercolumn)
