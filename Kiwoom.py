...
class Kiwoom(QAxWidget):
    ...
    def _event_connect(self, err_code):
        ...
        if self.testFlag:
            ...
            print("[_event_connect()] self.account = " + str(self.account))
            print("\n\n")
    ...
    def _receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3, unused4):
        ...
        if self.testFlag:
            print("[_receive_tr_data()] self.remainedData : " + str(self.remainedData))
            print("\n\n")

        self.testFlag = True
        #try:            
        self.queryProcess(trcode, rqname)

        # rqpool이 20개 초과인 경우, 가장 오래된 항목 삭제
    ...
    def _receive_real_data(self, code, realType, realData):
        ...
        if self.testFlag:
            ...
            print('[_receive_real_data()] type(realType) : ' + str(type(realType)))
            print('[_receive_real_data()] realData : ' + str(realData))
            print('[_receive_real_data()] type(realData) : ' + str(type(realData)))

        try:
            realdata = self.getRealData(code, realType, ['현재가', '누적거래량'])

            # realpool 에 종목코드 : [현재가, 누적거래량] 저장
            self.realpool[code] = [realdata['현재가'], realdata['누적거래량']]

            if self.testFlag:
                print('[_receive_real_data()] self.realpool[code] : ' + str(self.realpool[code]))
                print("\n\n")
    ...
    def _receive_chejan_data(self, gubun, itemCnt, fidListStr):
        ...
        if self.testFlag:
            ...
            print('[_receive_chejan_data()] type(gubun) : ' + str(type(gubun)))
        ...
        # chejanpool에 {fid의미:값} 의 dic 저장
        chejanResult = {}
        
        for fid in fidList:
            fidMeaning = self.convertFid(fid)
            if fidMeaning!=None: chejanResult[fidMeaning] = self.getChejanData(fid)

        self.chejanpool.append(chejanResult)
        
        if self.testFlag:
            print("\n")
            print('[_receive_chejan_data()] chejanResult : ')
            print(chejanResult)
            

        # chejanpool이 10개 이상이면 1개씩 비우고 chejan.txt 파일에 저장
        if len(self.chejanpool)>10:
            self.saveChejan(self.chejanpool[0])
            self.chejanpool = self.chejanpool[1:]

        self.finishEventLoop()
    ...
    def _receive_condition_ver(self, ret, msg):
        ...
        self.conditionNameList = self.getConditionNameList()

        if self.testFlag:
            print("[_receive_condition_ver()] type(self.conditionNameList) : " + 
                  str(type(self.conditionNameList)))
            print("[_receive_condition_ver()] self.conditionNameList : ")
            print(self.conditionNameList)
            print("\n\n")
    ...
    def _receive_tr_condition(self, screenNo, codeList, conditionIndex, next):
        ...
        if self.testFlag:
            print("[_receive_tr_condition()] self.remainedData : " + str(self.remainedData))

        result = codeList.split(';')
        self.conditionResult[conditionIndex] = result
        
        if self.testFlag:
            print("[_receive_tr_condition()] self.conditionResult : ")
            print(self.conditionResult)
            print("\n\n")
...
######################################################################################
    ...
    # 0을 반환하면 정상처리
    def commRqData(self, rqname, trcode, next, screen_no, outputs = []):
        ret = self.dynamicCall("CommRqData(QString, QString, int, QString)", rqname, trcode, next, screen_no)
        if ret!=0 : return ret

        # outputs == [] 이면 모든 가능한 output 얻기.
        # validOutputs는 해당 trcode에서 실제로 얻을 수 있는 출력들.
        validOutputs = self.getValidOutputs(trcode, outputs)
        self.rqpool[rqname] = validOutputs

        if self.testFlag:
            print("[commRqData()] validOutputs : ")
            print(validOutputs)
            print("\n\n")
    ...
    def commKwRqData(self, codeList, next, codeCount, typeFlag, rqname, screenNo, outputs=[]):
        ...
        # outputs == [] 이면 모든 가능한 output 얻기.
        # validOutputs는 해당 trcode에서 실제로 얻을 수 있는 출력들.
        validOutputs = self.getValidOutputs('OPTKWFID', outputs)
        self.rqpool[rqname] = validOutputs
...
######################################################################################
    ...
    def _opt10081(self):
        ...
        # 멀티데이터 [주식일봉차트조회]
        outputM = ["종목코드", "현재가", "거래량", "거래대금", "시가", "고가", "저가", "수정주가구분", "수정비율",
                   "대업종구분", "소업종구분", "종목정보", "수정주가이벤트", "전일종가"]
...
######################################################################################
    ...
    # 쿼리 결과를 rqpool의 해당 rqname 에 DataFrame 형식으로 저장
    def queryProcess(self, trcode, rqname):
        
        repeatCnt = self.getRepeatCnt(trcode, rqname)

        if self.testFlag:
            print("\n\n")
            print("[queryProcess()] trcode : " + trcode)
            print("[queryProcess()] rqname : " + rqname)
            print("[queryProcess()] repeatCnt : " + str(repeatCnt))

        indices = range(repeatCnt+1)
        
        outputs = self.rqpool.get(rqname)
        if outputs==None: outputs = self.getValidOutputs(trcode, [])
        ...
        if self.testFlag:
            print("\n")
            print("[queryProcess()] indices : " + str(indices))
            print("[queryProcess()] outputs : " + str(outputs))

        for i in indices:
            for o in outputs:
                df.set_value(i, o, self.getCommData(trcode, rqname, i, o))

        if self.testFlag:
            print("\n")
            print("[queryProcess()] df : ")
            print(df)
        
        self.rqpool[rqname] = df


    # 해당 rqname의 쿼리가 끝났는지 여부 반환
    def isQueryFinished(self, rqname):

        if self.testFlag:
            print("[isQueryFinished()]")
            print("\n\n")
        
        if type(self.rqpool.get(rqname))==type(DataFrame()): return True
        return False


    # 해당 rqname의 쿼리 결과를 반환하고, pool에서 제거
    def getRqResult(self, rqname):
    ...
    def getRealpool(self):
        ...
        ret = self.realpool.copy()
        return ret
        

    # chejanpool list를 반환
    def getChejanpool(self):
        
        if self.testFlag:
            print("[getChejanpool()]")
            print("\n\n")
        
        return self.chejanpool.copy()
    

    # chejanpool list 중 가장 오래된 것 반환
    ...
    # 버려지는 chejan 데이터를 chejan.txt에 저장
    def saveChejan(self, chejandic):
        
        try:
            fp = open('chejan.txt', 'a')
            fp.write('\n')

            for k, v in chejandic.items():
                fp.write(self.convertFid(k) + ' : ' + str(v) + '\n')

                if self.testFlag:
                    print("\n")
                    print("[saveChejan()] k : " + str(k))
                    print("[saveChejan()] v : " + str(v))

            fp.close()

        except:
            print("[saveChejan()] Can't save chejan data!!!")
